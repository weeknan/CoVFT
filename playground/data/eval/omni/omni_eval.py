...
def process(line, args, tokenizer, image_processor, model_config):
    ...
    context_prompt = qs.replace("<image>\n", "")
    #context_prompt = context_prompt.replace(f"\n{args.question_extension}", "")
    cleaned = re.sub(r"\([A-Z]\).*", "", context_prompt)

    cleaned = "\n".join([line.strip() for line in cleaned.splitlines() if line.strip()])
    cleaned = cleaned.replace("Estimate the real-world distances between objects in this image.", "")
    context_prompt = cleaned
    
    print(context_prompt)

    input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()

    return input_ids, image_tensor, image_size, prompt, context_prompt
...
def eval_model(args):
    ...
    questions = load_dataset("nyu-visionx/CV-Bench", split="test")
    questions = questions.filter(lambda example: example['type'] == '3D')
    # questions = []
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, image_tensor, image_sizes, prompt, context_prompt = process(line, args, tokenizer, image_processor, model.config)
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                ...
                use_cache=True,
                conversations_context=[context_prompt])
